...
from lib.api import get_obj, post_obj, patch_obj, delete_obj
from lib.utils import options, id_


def generic_orm(model, create_form=None, update_form=None, header=None):
    """
    Generic ORM operation interface.
    """
...
def generic_create(model, form=None):
    """
    ...
    if submit:
        response = post_obj(model, data)
        st.write(f'`POST {response.url}`', response, response.json())


def generic_retrieve(model, label='name'):
    """
    ...
    if submit:
        response = get_obj(model, obj_id)
        st.write(f'`GET {response.url}`', response, response.json())


def generic_update(model, form=None):
    """
...
def generic_delete(model, label='name'):
    ...
    if submit:
        if obj_id:
            response = delete_obj(model, obj_id)
